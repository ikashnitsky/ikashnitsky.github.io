...
Several days ago I saw [a post on LinkedIn by Jornt Mandemakers][orig] with some very curious results from [Gender and Generations Programme][ggp] surveys. While representing interesting data, it was a perfect example of a way too common academic dataviz fallacy that I decided to write this blog post. 

Here is the original plot by Jornt.

![](plot-to-redo.jpg)

We are going to replicate it and then redo -- all in hope to illustrate how much it can be improved with very very simple alterations. In fact, this post revives the argument I made in an earlier post called [Dotplot -- the single most useful yet largely neglected dataviz type][early]. Despite the nice title and the same  main message, I'm afraid the older post used a less indicative example. So I hope this time I'll manage to illustrate the key message a bit clearer. But the message itslf stays the same -- **forget multi-category bar/column plots and use dotplots instead**. Or, put it the other way around: whenever you are showing a continuous measurement split into multiple categories, place the continuous variable on the horizontal (x) axis and the categorical variable on the vertical (y) axis. That's it. Just a simple trick improves the readability of your plot **a lot**. 

Unfortunately, the use of crumpled multi-category bar plots is very widespread in academic papers. Just have a look at a Google Lens search on the focal plot in this post -- it just browses the internet for similarly looking images, and they are plenty, most of them coming straight from academic papers. 

![](google-lens.png)

# Rotate the damn plot

Over years of [teaching dataviz][artskill] to researchers I came to understanding that this particular mistreatment of data may be the most common and easily avoidable dataviz fallacy. Another dated attempt to spread this simple knowledge yuilded an educational datatviz aimed at the general audience. Here it is.
...
Okay. Enough with the lengthy intro. Let's replicate the plot from the very beginning of the post and then redo it. 

# Digitizing the data

But first we need the data from the plot. Of course, I could have taken it from the original source, [Gender and Generations Programme][ggp]. Or I could have asked the authors to share just the data used for this plot. But for simple plots that use a handful of data points there is a better simple alternative -- digitizing the values from the plot itself. There are numerous software solutions that streamline the task, including an [R package {digitize}][digitize]. This time I used a brilliantly simple [WebPlotDigitizer][wpd]. The idea of digitizing is very simple -- we just read the actual values from the plot and thus convert an image into the data underlying it. To do that we only need to provide the information about the coordinates of the plot grid.

![](digi-1.png)

Then the remaining task is just to click all the data points in already known coordinate space.

![](digi-2.png)

Just 35 clicks, and we are ready to export a clean CSV file with the data of the plot. For replocability of this post I uploaded the data and code to a [gist][gist]. 
...
The following lines of code take care of {ggplot2} theme, setting it to the one with my preferred selection of options. Earlier I wrote [a dedicated blog post][theme] for this portable theming solution. 
...
Note that the essential difference here is that I switched the axes effectively **rotating the damn plot**. I also used dots instead of filled bars. There is also a great advantage in doing this, which is nicely illustrated by this plot. Using dots alows to trim the continuous axis to zoom closer to the data. When you do the same to a bar plot, the visual impression gets distorted via the disproportional changes to the areas of the filled bars. Claus Wilke points this out and explains more in his brilliant [dataviz book][wilke].  
